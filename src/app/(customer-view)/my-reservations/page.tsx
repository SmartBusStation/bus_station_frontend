        ...
        <div className="min-h-screen p-3">
            ...
            <div className="lg:max-w-7xl max-w-5xl mx-auto">
                {hook.filteredTrips?.length > 0 ? (
                    <>
                        <div className={hook.viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  gap-6" : "space-y-3"}>
                            {hook.filteredTrips.map((trip: ReservationDetails, index: number) => {
                                const Component = hook.viewMode === "grid" ? GridCardTrip : TripListItem;
                                return (
                                    <Component
                                        ...
                                        onViewDetails={() => hook.navigateToDetails(trip.reservation.idReservation)}
                                    />
                                );
                            })}
                        </div>
